...
#ifdef TARGET_X86_64
#define GDB_CORE_XML "64bit-core.xml" 

//increment after adding new regs in xml definition
#define NUM_CORE_REGS  41 //41
static const int gpr_map[16] = {
    R_EAX, R_EBX, R_ECX, R_EDX, R_ESI, R_EDI, R_EBP, R_ESP,
    8, 9, 10, 11, 12, 13, 14, 15
};
#else
#define GDB_CORE_XML "32bit-core.xml" 
#define gpr_map gpr_map32
#define NUM_CORE_REGS  35 //32
#endif
static const int gpr_map32[8] = { 0, 1, 2, 3, 4, 5, 6, 7 };

#define IDX_IP_REG      CPU_NB_REGS                     //16     8
#define IDX_FLAGS_REG   (IDX_IP_REG + 1)                //17     9
#define IDX_SEG_REGS    (IDX_FLAGS_REG + 1)             //18     10
#define IDX_FP_REGS     (IDX_SEG_REGS + 6)              //24     16
//#define IDX_XMM_REGS    (IDX_FP_REGS + 16)              //40
//#define IDX_MXCSR_REG   (IDX_XMM_REGS + CPU_NB_REGS)    //56
...
static int cpu_gdb_read_register(CPUX86State *env, uint8_t *mem_buf, int n)
{
    ...
    if (n < CPU_NB_REGS) {
        ...
        case IDX_FP_REGS + 16: GET_REG32(env->idt.base); /* idtr_base */
        case IDX_FP_REGS + 17: GET_REG16(env->idt.limit); /* idtr_limit */
        case IDX_FP_REGS + 18: GET_REG32(env->cr[3]);
...
static int cpu_gdb_write_register(CPUX86State *env, uint8_t *mem_buf, int n)
{
    ...
    if (n < CPU_NB_REGS) {
        ...
        case IDX_FP_REGS + 16: return 4; /* idtr_base */
        case IDX_FP_REGS + 17: return 2; /* idtr_limit */
        case IDX_FP_REGS + 18: return 4; /*TODO SET?*/
...
static int gdb_handle_packet(GDBState *s, const char *line_buf)
{
    ...
    case 'm':
        ...
        if(read_raw_memory == 0){
            ...
            cpu_physical_memory_read(addr, mem_buf, len);
            memtohex(buf, mem_buf, len);
            put_packet(s, buf);
        }
	...
	if (strncmp(p, "Search", 5) == 0)
	{
#define BUFF_SIZE 0x1000
//		unsigned BUFF_SIZE=0x1000;
		void* search_buff;
		void* mem_offset;
		void* pattern;
		void* pattern_dec;
		unsigned addr, len, curr_addr;
		unsigned pat_len;
		bool found = 0;
		unsigned i, offset;
		char c[3];

		//get addr, len & pattern
		p += 14;
		addr = strtoull(p, (char **)&p, 16);
		p ++;
	        len = strtoull(p, (char **)&p, 16);
		...
		pattern_dec = malloc(pat_len/2 +1);
		strcpy(pattern, p);
		((char*)pattern)[pat_len] = 0; //lol?


		for(i=0; i< pat_len; i+=2)
		{
			c[2] = 0;
			c[1] = ((char*)pattern)[i+1];
			c[0] = ((char*)pattern)[i];
			((char*)pattern_dec)[i/2] = strtol(c, c[2], 16);
		}

		((char*)pattern_dec)[pat_len/2] = 0;

		if(len <= 0) break;

		if(0 == (search_buff = malloc(BUFF_SIZE))) break;

		curr_addr = addr;
		for(curr_addr = addr; curr_addr + (BUFF_SIZE - pat_len) < addr + len; curr_addr += (BUFF_SIZE - pat_len))
		{
//			printf("Searching 0x%x through 0x%x\n", curr_addr, curr_addr + BUFF_SIZE);
//			target_memory_rw_debug(s->g_cpu, curr_addr, search_buff, BUFF_SIZE, 0);
			cpu_physical_memory_read(curr_addr, search_buff, BUFF_SIZE);
			mem_offset = memmem(search_buff, BUFF_SIZE, pattern_dec, pat_len/2);
			if(mem_offset == 0x0)
				continue;
			offset = (unsigned)(mem_offset-search_buff+curr_addr);
//			printf("Found at 0x%x\n", offset);
			snprintf(buf, sizeof(buf), "1,%x", offset);
			found = 1;
			break;
		}

		//last iteration
		if((!found) && (len % BUFF_SIZE))
		{
//			printf("Last iteration, searching 0x%x through 0x%x\n", curr_addr, addr + len);
//			target_memory_rw_debug(s->g_cpu, curr_addr, search_buff, BUFF_SIZE, 0);
			cpu_physical_memory_read(curr_addr, search_buff, BUFF_SIZE);
			mem_offset = memmem(search_buff, addr + len - curr_addr - 1, pattern_dec, pat_len/2);
			if(mem_offset == 0x0)
				found = 0;
			else
			{
...
//			printf("Not found\n");
        ...
        report_cpuinfo:
            ...
            // printf("%s\n", p+5);
            // printf("%s\n", mem_buf);
            // printf("%d %d\n", strncmp(p + 5, "USE_PHYS_MEM", 12) == 0?1:0, strncmp(mem_buf, "USE_PHYS_MEM", 12) == 0?1:0);

            if(strncmp(mem_buf, "USE_PHYS_MEM", 12) == 0){
//                printf("%s\n", "USE_PHYS_MEM set.");
                read_raw_memory = 1;
            }else if(strncmp(mem_buf, "USE_VIRT_MEM", 12) == 0){
//                printf("%s\n", "USE_VIRT_MEM set.");
                read_raw_memory = 0;
            }else{
