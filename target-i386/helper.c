...
#ifdef TARGET_X86_64
#define SSE_XML_FILE "64bit-sse.xml"
#else
#define SSE_XML_FILE "32bit-sse.xml"
#endif



static int sse_gdb_get_reg(CPUX86State *env, uint8_t *mem_buf, int n){
    if (n < 16) {
        if (n < CPU_NB_REGS32 ||
        ...
        stl_p(mem_buf, env->mxcsr); 
        return 4; 
    }
    return 0;
}

static int sse_gdb_set_reg(CPUX86State *env, uint8_t *mem_buf, int n){
    if (n < 16) {
        if (n < CPU_NB_REGS32 ||
            (TARGET_LONG_BITS == 64 /*&& env->hflags & HF_CS64_MASK*/)) {
...
X86CPU *cpu_x86_init(const char *cpu_model)
{
    ...
    gdb_register_coprocessor(env, sse_gdb_get_reg, sse_gdb_set_reg, CPU_NB_REGS + 1, SSE_XML_FILE, 0); //xmm0..n + mxcsr
    if (error) {
        error_free(error);
        object_unref(OBJECT(cpu));
        return NULL;
